...
if ($_SESSION["aceAccessLevel"] == 3) {
	if ($currentSemesterID == 0) {
		$sql ="SELECT sl.serviceLearnerID as serviceLearnerID, sl.serviceLearnerUsername as serviceLearnerUsername, sc.sectionID as sectionID, sc.sectionNumber as sectionNumber, s.clockIn as clockIn, s.clockOut as clockOut, s.missedIn as missedIn, s.missedOut as missedOut, s.shiftComment as shiftComment 
		FROM SHIFT AS s, SERVICE_LEARNER AS sl, SECTION as sc
		WHERE s.serviceLearnerID = sl.serviceLearnerID AND 
		s.sectionID = sc.sectionID;";
	} else {
		$sql ="SELECT sl.serviceLearnerID as serviceLearnerID, sl.serviceLearnerUsername as serviceLearnerUsername, sc.sectionID as sectionID, sc.sectionNumber as sectionNumber, s.clockIn as clockIn, s.clockOut as clockOut, s.missedIn as missedIn, s.missedOut as missedOut, s.shiftComment as shiftComment 
		FROM SHIFT AS s, SERVICE_LEARNER AS sl, SECTION as sc
		WHERE s.serviceLearnerID = sl.serviceLearnerID AND 
...
echo("
        ...
        <div id='page-wrapper'>
            <div class='container-fluid'>
                <div class='row'>
                    <div class='col-lg-12'>
                        <h1 class='page-header'>Shift List</h1>
                    </div>
                </div>");
 ...
 if ($var_success == 'updated') {
  ...
  if ($var_success == 'delete') {
   ...
   ");
				...
				echo("<div class='row'>
					<div class='col-lg-12'>
					<p class='text-right'><a role=\"button\" class=\"btn btn-lg btn-success\" href='addShift.php'>Add Shift </a></p>
...
if (mysqli_num_rows($result) > 0) {
	while($row = mysqli_fetch_assoc($result)) {
		echo("['" . $row["serviceLearnerUsername"] . "',
				'" . $row["sectionNumber"] . "',
				'" . $row["clockIn"] . "',
				'" . $row["clockOut"] . "',
                '" . $row["missedIn"] . "',
                '" . $row["missedOut"] . "',
                '" . $row["shiftComment"] . "',
				'<form action=\"editShift.php\" method=\"get\" style=\"display:inline\"> <button type=\"submit\" class=\"btn btn-primary btn-xs\" name=\"shiftID\" value=\"". $row["shiftID"] ."\">Edit</button> </form>'],");
	};
}; 
