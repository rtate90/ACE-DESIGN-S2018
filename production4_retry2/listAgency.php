...
$sql = "SELECT a.agencyID as aID, a.agencyName as aName 
FROM AGENCY as a;";
...
echo("
            ...
            <div class='row'>
                <div class='col-lg-12'>
                <p class='text-right'><a role=\"button\" class=\"btn btn-lg btn-success\" href='addAgency.php'> Add New Agency </a></p>
                    <div class='panel panel-default'>
                        <!-- /.panel-heading -->
                        <div class='panel-body'>
                            <table width='100%' class='table table-striped table-bordered table-hover' id='dataTables-agency'>
                            </table>
                        </div>
                    </div>
                </div>
	...
	");
      if (mysqli_num_rows($result) > 0) {
          while($row = mysqli_fetch_assoc($result)) {
              echo "['" . $row["aName"] . "',
              '<form action=\"editAgency.php\" method=\"get\" style=\"display:inline\"> <button type=\"submit\" class=\"btn btn-primary btn-xs\" name=\"agencyID\" value=\"" . $row["aID"] .
              "\">Edit</button> </form>'],";
		  };
      } else {
          echo "['0 results']";
