 ...
 if ($var_success == 'updated') {
  ...
  if ($var_success == 'deleted') {
   ...
   ");
            ...
            echo("<div class='row'>
                <div class='col-lg-12'>
                <p class='text-right'><a role=\"button\" class=\"btn btn-lg btn-success\" href='addFaculty.php'> Add New Faculty </a></p>
    ...
    var dataSet = [
      ...
      if (mysqli_num_rows($result) > 0) {
          while($row = mysqli_fetch_assoc($result)) {
              echo "['" . $row["fName"] . "',
              '" . str_replace("'", "", $row["lName"]) . "',
			  '<form action=\"editFaculty.php\" method=\"get\" style=\"display:inline\"> <button type=\"submit\" class=\"btn btn-primary btn-xs\" name=\"facultyID\" value=\"" . $row["facultyID"] ."\">Edit</button> </form>'],";
          };
      } else {
          echo "['0 results']";
      };
